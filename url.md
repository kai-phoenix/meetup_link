...
flowchart LR
    ...
    LOGIN_P --> |認証成功| TOP_G["GET /events<br/>トップページ"]
    ...
    TOP_G --> |リダイレクト| POST_REGISTER_C["GET /events/create<br/>イベント情報新規登録画面"]
    POST_REGISTER_C --> |登録| POST_P["POST /events<br/>イベント情報登録"]
    TOP_G --> |リダイレクト|POST_EDIT_G["GET /events/{id}/edit<br/>イベント情報更新画面"]
    POST_EDIT_G--> |更新| POST_EDIT_PU["PUT /events/{id}<br/>イベント情報更新"]
    TOP_G --> |キャンセル| POST_CANCEL_P["POST  /events/{id}/cancel<br/>イベント情報キャンセル"]
    POST_P --> |メール送信| TOP_G
    POST_EDIT_PU --> |メール送信| TOP_G
    POST_CANCEL_P --> |キャンセルメール| TOP_G
    TOP_G --> |予約作成| POST_RESERVE_POST["POST /events/{id}/reserve<br/>イベントへ予約作成"]
    POST_RESERVE_POST --> |予約不可または完了時メール送信|TOP_G
    TOP_G --> |予約キャンセル| POST_RESERVE_D["DELETE /events/{id}/reserve<br/>イベント予約キャンセル"]
